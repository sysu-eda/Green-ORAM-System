...
long long int labelAllocation_cnt=0;
long long int pathReplacement_cnt1=0;
long long int pathReplacement_cnt2=0;
long long int fork_pathReplacement_cnt1=0;
long long int fork_pathReplacement_cnt2=0;
long long int noORAMreadnum=0;
long long int newdatacnt=0;
long long int newnaivedatacnt=0;
...
int get_read_num_oram_single(long long int physical_address)
{
  ...
  labelAllocation_cnt+=joint_path[this_label];
  ...
  while (!expt_done) {
    ...
    for (numc = 0; numc < NUMCORES; numc++) {
	  ...
	  if (nonmemops[numc]) {  /* Have some non-memory-ops to consume. */
        ...
        // int read_num=get_read_num_oram_single(addr[numc]);
        // noORAMreadnum++;
        // if(read_num!=0)
